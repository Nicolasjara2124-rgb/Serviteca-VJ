...
import { 
  ...
  History,
  Barcode,
  Loader2 // <-- Añadido para el ícono de carga
} from 'lucide-react';
...
interface InventarioProps {
  ...
  sucursales: Sucursal[];
  usuario: Usuario;
  // Actualizamos estas 3 funciones para que acepten Promesas asíncronas de Firebase
  onSaveProducto: (prod: Producto) => Promise<void> | void;
  onDeleteProducto: (id: string) => Promise<void> | void;
  onTransferirStock: (trans: TransferenciaStock) => Promise<void> | void;
...
export default function Inventario({ 
  ...
  const [isSaving, setIsSaving] = useState(false); // <-- Nuevo estado para controlar la carga en la nube
  ...
  // 4. Guardar producto (Nuevo o Editado) - AHORA ASÍNCRONO
  const handleSaveProductoSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    setIsSaving(true); // Iniciamos animación de carga
    ...
    try {
      await onSaveProducto(nuevoProd); // Esperamos a Firebase
      setShowProductModal(false);
    } catch (error) {
      console.error("Error al guardar el producto:", error);
      alert("Hubo un error al guardar en la nube. Intente nuevamente.");
    } finally {
      setIsSaving(false);
    }
  };

  // 5. Eliminar Producto - AHORA ASÍNCRONO
  const handleDeleteProductoClick = async (id: string, nombre: string) => {
    if (confirm(`¿Está seguro de eliminar el producto/servicio "${nombre}" del catálogo?`)) {
      try {
        await onDeleteProducto(id); // Esperamos a Firebase
      } catch (error) {
        console.error("Error al eliminar:", error);
        alert("Hubo un error al eliminar el producto en la nube.");
      }
    }
  };
  ...
  // 7. Enviar Transferencia de Stock - AHORA ASÍNCRONO
  const handleTransferirSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onTransferirStock(nuevaTransferencia); // Esperamos a Firebase
      setShowTransferModal(false);
    } catch (error) {
      console.error("Error al transferir stock:", error);
      alert("Hubo un error al procesar el traspaso en la nube.");
    } finally {
      setIsSaving(false);
    }
  };
          ...
          <form onSubmit={handleSaveProductoSubmit} className="bg-slate-900 border border-slate-800 text-slate-100 w-full max-w-xl rounded-2xl p-6 shadow-2xl relative animate-scaleUp">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                ...
                <div className="relative flex items-center bg-slate-950 border border-slate-800 rounded-xl focus-within:border-blue-500 transition-colors">
                  <input
                    ...
                    className="w-full bg-transparent px-3 py-2 text-xs text-white focus:outline-none font-mono disabled:opacity-50"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-white focus:border-blue-500 focus:outline-none disabled:opacity-50"
                >
                  {categorias.map(cat => (
              ...
              {prodCategoria !== 'Servicios' ? (
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-white focus:border-blue-500 focus:outline-none font-mono disabled:opacity-50"
                  />
                </div>
              ) : (
                <div className="space-y-1">
                  ...
                  {sucursales.map(s => (
                    <div key={s.id} className="space-y-1">
                      ...
                      <input
                        ...
                        className="w-full bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1.5 text-xs text-white text-center font-mono focus:border-blue-500 focus:outline-none disabled:opacity-50"
                      />
                    </div>
                  ))}
                </div>
            ...
            <div className="flex gap-2 justify-end mt-6 border-t border-slate-850 pt-4">
              <button
                ...
                className="py-2 px-4 bg-slate-850 hover:bg-slate-800 text-slate-400 font-semibold text-xs rounded-xl border border-slate-800 transition-all disabled:opacity-50"
              >
                Cancelar
              </button>
              <button
                type="submit"
                disabled={isSaving}
                className="py-2 px-5 bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs rounded-xl shadow-lg shadow-blue-500/10 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
                {isSaving ? (
                  <>
